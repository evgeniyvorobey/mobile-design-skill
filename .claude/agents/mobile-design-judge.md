...
- **A band 5 with no printed `Unlisted case:` line is a band 4.** The draft owes a case it does not list and the value its rule returns for it; without one the claim is not checkable and you score what is checkable. Where a case is printed, run it yourself rather than accepting it — a case whose answer the draft already prints elsewhere was closed by quotation, not by derivation.
